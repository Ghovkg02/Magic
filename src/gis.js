...
mapboxgl.accessToken = 'pk.eyJ1IjoiYmliaW5wIiwiYSI6ImNsNTg4eGhuMTBuaWUzY25zZG9jOXl6c2MifQ.QvrYsftaHTXkCNq2b5ytPQ';
...
const setCoordButton = document.getElementById('set-coord');
const inputLng = document.getElementById('input-lng');
const inputLat = document.getElementById('input-lat');

setCoordButton.addEventListener('click', () => {
    const newLng = parseFloat(inputLng.value);
    const newLat = parseFloat(inputLat.value);

    console.log('New Coordinates:', newLng, newLat); // Debugging line

    if (!isNaN(newLng) && !isNaN(newLat)) {
        console.log('Valid coordinates, updating marker...');
        lng = newLng;
        lat = newLat;
        coordinates = [lng, lat];
        ...
        marker1.getPopup().setHTML(`<h3>current location:</h3><h5>[${lng}, ${lat}]</h5>`);
